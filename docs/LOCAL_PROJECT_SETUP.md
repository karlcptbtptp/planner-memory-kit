...
目标：让每个策划项目接入共享记忆时，都有同一套本地组织方式。拿到 Git 工具仓和飞书多维表格后，项目里只需要搭三块：
...
project-root/
  ...
  03_memory/                  # 智能体记忆
    README.md
    shared/                   # 从飞书主库同步下来的团队知识缓存
      README.md
      feishu-memory.md        # 自动生成，不手写
...
- 飞书同步下来的团队共享知识：`03_memory/shared/feishu-memory.md`
...
- `03_memory/shared/feishu-memory.md` 由同步命令生成，不手写。
- `03_memory/personal/` 默认加入 `.gitignore`。
- 能复用给团队的知识，回到飞书主库投稿并走维护人审核。
...
| `03_memory/shared/feishu-memory.md` | 第一版默认不提交，除非团队明确要做脱敏镜像 |
