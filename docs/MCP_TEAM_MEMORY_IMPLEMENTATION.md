...
把团队共享记忆从“飞书多维表格人工主库”升级为“远端 MCP 团队记忆库 + 本机 AI 高速缓存”。
...
| 本机 SQLite | 每个项目本地 | AI 运行缓存、全文搜索、任务记忆包、使用事件明细 |
...
3. 验证只出现全项目原则和当前项目知识。
4. 真实同步后，本地搜索能召回。
5. 记录一次 adopt 和一次 miss。
6. 回传 usage 后，MCP 端健康度能看到聚合反馈。
7. 将当前项目知识冻结，再同步一次，本地不应继续默认召回。
